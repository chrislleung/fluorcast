#!/bin/bash
#SBATCH --job-name=graph_gcn_gpu
#SBATCH --partition=gpubase_bygpu_b2
#SBATCH --time=12:00:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --gpus-per-node=h100:1
#SBATCH --output=outputs/slurm/graph_gcn_emission_gpu_%j.out
#SBATCH --error=outputs/slurm/graph_gcn_emission_gpu_%j.err
...
mkdir -p models/graph_gcn_emission_gpu
mkdir -p outputs/graph_gcn_emission_gpu
...
echo "GPU graph GCN emission job started on $(hostname)"
...
echo "Running graph_gcn emission_nm on GPU..."
python -u scripts/run_graph_model_experiments.py \
  ...
  --out-root models/graph_gcn_emission_gpu \
  --compare-out outputs/graph_gcn_emission_gpu \
  --models graph_gcn \
  --targets emission_nm \
  --epochs 50 \
  --patience 8 \
...
echo "End time: $(date)"
echo "GPU graph GCN emission job completed."

echo "Outputs:"
ls -lh outputs/graph_gcn_emission_gpu || true
