#!/bin/bash
#SBATCH --job-name=gcn_qy_gpu
#SBATCH --partition=gpubase_bygpu_b2
#SBATCH --time=12:00:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --gpus-per-node=h100:1
#SBATCH --output=outputs/slurm/graph_gcn_qy_gpu_%j.out
#SBATCH --error=outputs/slurm/graph_gcn_qy_gpu_%j.err
...
mkdir -p models/graph_gcn_qy_gpu
mkdir -p outputs/graph_gcn_qy_gpu
...
echo "Graph GCN quantum_yield GPU job started on $(hostname)"
...
python -u scripts/run_graph_model_experiments.py \
  ...
  --out-root models/graph_gcn_qy_gpu \
  --compare-out outputs/graph_gcn_qy_gpu \
  --models graph_gcn \
...
echo "End time: $(date)"
echo "Graph GCN quantum_yield GPU job completed."
ls -lh outputs/graph_gcn_qy_gpu || true
